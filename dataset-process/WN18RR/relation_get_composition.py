...
def process_entities(file_path):
    # 打开并读取JSON文件
    fie = "relation.json"
    with open(fie, 'r', encoding='utf-8') as file:
        data = json.load(file)
    ...
    for item in data:
        ...
        messages = "在知识图谱本体框架中组合关系是指，如果A和B之间存在关系R1，B和C之间存在关系R2，且A和C之间存在关系R3，则关系R3等于关系R1和关系R2的组合，其中关系R3可以是R1和R2的任意一个，也可以是一个新的关系。当前关系R1为" + relation + "请帮我从下面的关系集中找到关系B和关系C组成组合关系，并且严格按照" + json.dumps(json_string).strip('"') + "的格式进行输出，不要输出任何其他东西。\n下面是你需要分析的关系集：\n" + strr
        llm_result = algorithm(messages)

        ans = remove_think_part(llm_result) # 去掉 think
        print(ans)
        item['composition'] = ans
...
process_entities('relation_composition.json')
