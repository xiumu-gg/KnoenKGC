...
def process_entities(file_path):
    ...
    entities = [{"entity": item["freebase_id"]} for item in data]
    # 每十个一组调用algorithm函数
    group_size = 10
    res = []
    process = ""
    ...
    for i in range(0, len(entities), group_size):
        if len(entities) > i + group_size:
            ...
            json_string = '[{"entity":"entity1"},{"entity":"entity2"}]'
            json_string2 = '[{"entity":"entity1"， "name":"entity name1"},{"entity":"entity2"， "name":"entity name2"}]'
            messages = "I will input ten entity symbols in the format of " + json.dumps(json_string).strip('"') + " each time. Could you help me generate their corresponding entity names in the ontology system? Please output in the format of " + json_string2 + ", and do not output any other irrelevant content." + "\n" + entity_inf
            print(messages)
            print(i)
        ...
        if json_result == None:
            ...
            res = res + json_result
        print(i)
        write_to_file(res, "entity_name.json")

# 假设你的数据存储在entity.json文件中
# process_entities('entity.json')
file_path = "entity_name.json"
file = "entity.json"
...
for i in range(len(data)):
    data1[i]["label"] = data[i]["name"]

write_to_file(data1, "entity.json")