...
def process_sym(data):
    # json_string = '{"relation":"relation name", "symmetry":""}'
    messages = "在知识图谱本体框架中对称关系是指，如果A和B之间存在关系R，B和A之间也存在关系R，则R为对称关系。请帮我分析关系" + data + "是否为互斥关系，如果是对称关系，请输出“true”，如果不是，请输出“false”。请不要输出“ture”和“false”之外的其他内容。"
    llm_result = algorithm(messages)
...
def relation_get(file_path):

    with open(file_path, 'r', encoding='utf-8') as file:
        data = json.load(file)
    ans = 0
    for item in data:
        if ("equiralence" in item) and (type(item["equiralence"]) == list):
            continue
    ...
    for item in data:
        if ("disjointness" in item) and (type(item["disjointness"]) == list):
            continue
        strr = get_str(data, item['id'])
    ...
    for item in data:
        if ("composition" in item) and (type(item["composition"]) == list):
            continue
        strr = get_str(data, item['id'])
    ...
    for item in data:
        if ("symmetry" in item):
            continue
        res = process_sym(item["freebase"])
        cleaned_data_str = res.strip()
        if "true" in cleaned_data_str:
            item["symmetry"] = Ture
            print(item['id'])
        else:
            item["symmetry"] = False
...
# for item in data:
#     item["label"] = item["label"][0]["result"]
...
while(aaa):
    ...
    aaa = False
    for item in data:
        if "equiralence" not in item:
            aaa = True
            break
        if "disjointness" not in item:
            aaa = True
            break
