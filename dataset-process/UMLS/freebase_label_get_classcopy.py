...
def get_classid(file_path):
    ...
    for item in data:
        ...
        if classname in classname_to_classid:
            item["classid"] = classname_to_classid[classname]
        else:
            classname_to_classid[classname] = ans
            ans += 1
            item["classid"] = classname_to_classid[classname]
            eee.append({"classname":classname, "classid":item["classid"]})

    # write_to_file(data, file_path)
    

    # write_to_file(eee, "class.json")
