...
def process_entities(file_path):
    ...
    for item in data:
        ...
        messages = "在知识图谱本体框架中互斥关系是指，如果A和B之间存在关系R1，A和B之间存在一定不存在关系R2，并且R1和R2大概率为反义词，则R1和R2为互斥关系。请帮我分析下面哪些关系和" + relation + "是互斥关系，并且严格按照" + json.dumps(json_string).strip('"') + "的格式进行输出，不要输出任何其他东西。\n下面是你需要分析的关系：\n" + strr
...
process_entities('data/relation_disjointness.json')
